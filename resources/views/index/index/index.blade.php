...
@section('content')
     ...
     @foreach ($cateInfo as $v)
      ...
      <option><a href="prolist" value="{{$v->cate_id}}">@php echo str_repeat('-',$v['lv'])@endphp {{$v->cate_name}}</a></option>
   @endforeach
  </select>
      <div class="clearfix"></div>
     </ul><!--pronav/-->
    ...
    <dl>
        <dt><a href="{{url('/proinfo')}}"><img src="{{env('UPLOAD_URL')}}{{$v->goods_img}}" width="100"></a></dt>
        <dd class="ip-text"><a href="{{url('/proinfo')}}">{{$v->goods_name}}</a><span>已售：77</span></dd>
